-- FinanceOS Migration v3 — Ejecutar en Supabase SQL Editor DESPUÉS de migration_v2.sql
...
-- 6. Usuarios base (rellenar PHONE_ANGEL y PHONE_ALICIA con números reales)
INSERT INTO usuarios (telefono, nombre, role, ai_preference, ai_model) VALUES
  ('whatsapp:+52NUMERO_ANGEL',  'Angel',  'ADMIN_A', 'CLAUDE', 'claude-sonnet-4-6'),
  ('whatsapp:+52NUMERO_ALICIA', 'Alicia', 'USER_B',  'GEMINI', 'gemini-1.5-flash')
ON CONFLICT (telefono) DO UPDATE SET
  ai_model = EXCLUDED.ai_model;

-- IMPORTANTE: reemplaza whatsapp:+52NUMERO_ANGEL con el número real antes de correr:
-- UPDATE tdc        SET user_phone = 'whatsapp:+52NUMERO_ANGEL' WHERE user_phone = '';
-- UPDATE metas      SET user_phone = 'whatsapp:+52NUMERO_ANGEL' WHERE user_phone = '';
-- UPDATE movimientos SET user_phone = 'whatsapp:+52NUMERO_ANGEL' WHERE user_phone = '';
